...
# --- هدية المساعد الذكي لأسامة (Osamh AI Assistant) ---
with st.expander("🤖 مساعد أسامة الذكي - اسألني أي شيء"):
    user_input = st.text_input("مرحباً بك! أنا مساعد أسامة الذكي، كيف يمكنني مساعدتك اليوم؟")
    if user_input:
        if "مباراة" in user_input or "كورة" in user_input:
            st.write("🤖 مساعد أسامة: يمكنك معرفة كافة مواعيد مباريات اليوم من خلال 'سيرفر الرياضة' في القائمة!")
        elif "اشتراك" in user_input or "باقة" in user_input:
            st.write(f"🤖 مساعد أسامة: أهلاً بك! الباقة الشهرية بـ 5$ والذهبية بـ 10$. تواصل مع المطور أسامة الفقيه على: 967775072692")
        elif "فيلم" in user_input or "سهرة" in user_input:
            st.write("🤖 مساعد أسامة: أنصحك بمتابعة قسم 'الأفلام'.. أسامة أضاف أفلاماً حصرية اليوم!")
        else:
            st.write("🤖 مساعد أسامة: شكراً لسؤالك! سأقوم بإبلاغ المطور أسامة بطلبك فوراً وسنرد عليك.")

# إضافة لمسة "البصمة التقنية" (إشعار متحرك)
st.toast('مرحباً بك في عالم أسامة الفقيه للتقنية 🚀', icon='🔥')
